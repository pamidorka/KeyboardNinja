...
ButtonColorSet kButtonColorSet
        = ButtonColorSet(sf::Color::Black, sf::Color::Black, sf::Color::Black);

CTEST(button_tests, mouse_hover)
{
    Button btn(30, 30, 20, 20, nullptr, "", 1, kButtonColorSet);

    ASSERT_TRUE(btn.IsMouseHover(sf::Vector2f(40, 40)));
    ASSERT_TRUE(btn.IsMouseHover(sf::Vector2f(48, 32)));
    ASSERT_FALSE(btn.IsMouseHover(sf::Vector2f(15, 40)));
    ASSERT_FALSE(btn.IsMouseHover(sf::Vector2f(35, 5)));
}

CTEST(button_tests, enter_and_left)
{
    Button btn(0, 0, 100, 100, nullptr, "", 1, kButtonColorSet);

    ASSERT_TRUE(btn.GetButtonState() == ButtonStates::ButtonIdle);
    ASSERT_FALSE(btn.Update(sf::Vector2f(500, 500), false));
    ASSERT_TRUE(btn.GetButtonState() == ButtonStates::ButtonIdle);
...
CTEST(button_tests, click_outside)
{
    ...
    ASSERT_FALSE(btn.Update(sf::Vector2f(500, 500), true));
    ASSERT_TRUE(btn.GetButtonState() == ButtonStates::ButtonIdle);
}
...
CTEST(button_tests, click_and_leave)
{
    ...
    ASSERT_FALSE(btn.Update(sf::Vector2f(10, 10), true));
    ASSERT_TRUE(btn.GetButtonState() == ButtonStates::ButtonHoverActive);
    ASSERT_FALSE(btn.Update(sf::Vector2f(500, 10), true));
    ASSERT_TRUE(btn.GetButtonState() == ButtonStates::ButtonUnhoverActive);
    ASSERT_FALSE(btn.Update(sf::Vector2f(500, 10), false));
    ASSERT_TRUE(btn.GetButtonState() == ButtonStates::ButtonIdle);
}
